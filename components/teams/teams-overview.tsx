...
export default function TeamsOverview() {
  ...
  const filteredUsersForMember = users.filter(user =>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto w-full">
            <Table className="min-w-full w-full">
              ...
              <TableBody>
                {filteredTeams.map(({ _id, teamName, eventName, leaderId, members, isRegistered, isVerified, teamCode }) => (
                  <TableRow key={_id}>
                    <TableCell>{teamName}</TableCell>
                    <TableCell>{eventName}</TableCell>
                    <TableCell>{getUserName(leaderId)}</TableCell>
                    <TableCell>
                      {members.length > 0 ? (
                        members.map(memberId => (
                          <div key={memberId}>{getUserName(memberId)}</div>
                        ))
                      ) : (
                        <span>No members</span>
                      )}
                    </TableCell>
                    <TableCell>{isRegistered ? "Yes" : "No"}</TableCell>
